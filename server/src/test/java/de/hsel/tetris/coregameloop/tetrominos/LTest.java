...
import static de.hsel.tetris.coregameloop.Color.ORANGE;
...
class LTest {
    ...
    private final Integer c = ORANGE.value;
    ...
    void shouldTakeUpCorrectCellsWhenPlaced() {
        var grid = new Grid();
        var tetromino = new L(0, grid.getHeight() - 3);
    ...
    @Test
    void shouldProvideLType() {
        var tetromino = new L(0, 0);
        assertEquals(Type.L, tetromino.getType());
    }
    ...
    void shouldSerializeAppropriately() {
        ...
        assertEquals(Type.L, result.type());
